package com.zihe.tams.module.coursescheduling.service.impl;
...
import com.zihe.tams.common.consts.CommonConsts;
import com.zihe.tams.common.consts.ErrorCodeEnum;
...
import com.zihe.tams.module.coursescheduling.model.convert.AbstractCourseSchedulingConverter;
...
public class CourseSchedulingServiceImpl extends ServiceImpl<CourseSchedulingMapper, CourseSchedulingDO> implements CourseSchedulingService {
    ...
    public List<CourseSchedulingListVO> listCourseScheduling(CourseSchedulingQuery query) {
        ...
        QueryWrapper<CourseSchedulingDO> queryWrapper = Wrappers.<CourseSchedulingDO>query()
                ...
                .eq("cs.is_deleted", 'N').orderByAsc("date");


        List<CourseSchedulingListVO> courseSchedulingListVOS = baseMapper.selectCourseSchedulingList(queryWrapper);
        courseSchedulingListVOS.forEach((vo) -> {
            Integer appointNum = courseAppointService.getAppointNum(vo.getId());
            Integer attendanceNum = courseAppointService.getAttendanceNum(vo.getId());
            vo.setAppointNum(appointNum);
            vo.setSignInNum(attendanceNum);
            // 转换一下picture_url
            String json = vo.getPictureUrl();
            JSONArray jsonArray = JSONUtil.parseArray(json);
            if (jsonArray.size() >= 1) {
                vo.setPictureUrl((String) jsonArray.get(0));
            } else {
                vo.setPictureUrl("http://101.133.146.88/lisaPic/logo.jpg");
            }
        });
        return courseSchedulingListVOS;
    }

    @Override
    public CourseSchedulingListVO getCourseSchedulingById(Long id) {

        CourseSchedulingListVO courseSchedulingListVO = baseMapper.selectCourseSchedulingById(id);
        List<Integer> list = (List<Integer>) JSONUtils.parse(courseSchedulingListVO.getCardType());
        List<String> cardTypeList = new ArrayList<>();
        for (int i = 0; i < list.size(); i++) {
            cardTypeList.add(CommonConsts.cardType.get(list.get(i)));
        }
        courseSchedulingListVO.setCardTypeList(cardTypeList);
    ...
    @Override
    public CourseSchedulingReportVO getReportOneTeacherCount(Long teacherId, String startDate, String endDate) {
        return baseMapper.selectReportTeacherCountOne(teacherId, startDate, endDate);
    }
    ...
    public boolean removeCourseSchedulingById(Long id) {
        courseAppointMapper.deleteLogic(id);
